...
if ($action == 'saveTeacher') {
    ...
    if ($id == '') {
        $teacherInsert = $conn->query(
            "INSERT INTO `create_teacher` (f_name, l_name, email, contact_num, address, gender, join_date) 
            VALUES ('$tFname', '$tLname', '$tEmail', '$tContact', '$tAddress', '$tGender', '$tDoJ')"
        );
        ...
        $teacherUpdate = $conn->query(
            ...
            contact_num = '$tContact', address = '$tAddress', gender = '$tGender', join_date = '$tDoJ' 
            WHERE Id = '$id'"
        );
